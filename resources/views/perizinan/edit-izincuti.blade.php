...
@section('content')
    <div class="row" style="margin-top: 5rem;">
        <div class="col">
            <form action="{{route('update-izincuti',['kode_izin' => $dataizin->kode_izin])}}" method="POST" class="ml-2 mr-2" id="formIzin">
                @csrf
                ...
                <div class="form-group">
                    <select name="kode_cuti" id="kode_cuti" class="form-select custom-select">
                        ...
                        <option value="{{$item->kode_cuti}}" {{ $dataizin->kode_cuti == $item->kode_cuti ? 'selected' : "" }}>{{$item->nama_cuti}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
